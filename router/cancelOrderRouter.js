...
router.post('/',(req,res) =>{
    ...
    for(let i  = 0; i <prod.length; i++){
        if(prod[i].productId == productId ){
            prod[i].quantity =Number(prod[i].quantity)+Number(quantity);
            product.set('productPost',prod);
        }
    }
    for(let i  = 0; i <trans.length; i++){
        if(trans[i].orderId == orderId ){
            trans[i].status = 'Cancelled';
            transact.set('tHistory',trans);
        }
    }
    
    req.flash('success',"Successfully cancelled Order!");
    res.redirect('/myOrderList')
});
